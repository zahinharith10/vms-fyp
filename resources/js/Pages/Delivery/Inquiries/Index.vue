...
const submitReply = (inquiryId) => {
    replyForm.post(route('delivery.inquiries.reply', inquiryId), {
        preserveScroll: true,
        onSuccess: () => {
            replyForm.reset();
        }
...
const endInquiry = (inquiryId) => {
    ...
    endForm.post(route('delivery.inquiries.end', inquiryId), {
        preserveScroll: true,
    });
};
        ...
        <div class="max-w-3xl mx-auto">
            ...
            <div class="space-y-4">
                ...
                <div
                    ...
                    class="bg-white dark:bg-gray-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 hover:border-orange-200 dark:hover:border-orange-850 transition-all duration-200 cursor-pointer"
                >
                            ...
                            <div v-if="inquiry.messages && inquiry.messages.length > 0" class="space-y-3 flex flex-col gap-2">
                                ...
                                >
                                    <div class="flex items-center gap-1.5 text-[8px] font-black uppercase tracking-wider mb-1"
                                        :class="msg.sender_type === 'User' ? 'text-orange-200' : 'text-gray-400 dark:text-gray-500'">
                        ...
                        <div v-if="inquiry.status !== 'Resolved'" class="space-y-4 pt-2">
                            ...
                            <form @submit.prevent="submitReply(inquiry.id)" class="space-y-2">
                                <textarea
                                    ...
                                    class="w-full bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 rounded-2xl text-xs font-medium text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all p-3"
                                    required
                                ></textarea>
                                <div class="flex justify-between items-center">
                                    ...
                                    <button
                                        ...
                                        class="px-5 py-2 bg-orange-600 hover:bg-orange-700 text-white font-black text-[10px] uppercase tracking-wider rounded-xl transition-all shadow-sm flex items-center gap-1.5 disabled:opacity-50"
                                    >

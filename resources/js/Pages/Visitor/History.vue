...
<template>
    ...
    <VisitorAuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">My Visit History</h2>
        </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4 sm:mb-0">All Visits</h3>
                    <div class="flex space-x-2">
                        <select v-model="filterStatus" class="border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">
                ...
                <div v-if="filteredVisits.length > 0" class="space-y-4">
                    <div v-for="visit in filteredVisits" :key="visit.id" class="bg-gray-50 border border-gray-200 rounded-lg p-5 flex flex-col sm:flex-row sm:items-center justify-between transition-all hover:shadow-md">
                        
                        <div class="mb-4 sm:mb-0">
                            <div class="flex items-center space-x-3 mb-1">
                                <span class="text-xl font-black text-indigo-900">{{ visit.unit_number }}</span>
                                <span :class="{
                        ...
                        <div class="flex-shrink-0 flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-3">
                            ...
                            <button 
                                ...
                                class="inline-flex items-center justify-center px-4 py-2 bg-red-50 border border-red-200 rounded-md font-semibold text-xs text-red-600 uppercase tracking-widest hover:bg-red-100 hover:border-red-300 transition ease-in-out duration-150"
                            >
                                Cancel
                            </button>

                            <span v-if="visit.status === 'Rejected' || visit.status === 'Checked Out'" class="text-sm text-gray-400 italic px-2">
                                No Actions
                            </span>
                        </div>
                    </div>
                ...
                <div v-else class="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
                    <p class="text-gray-500 font-medium text-lg">No visits found.</p>
                    <p class="text-sm text-gray-400 mt-1">You haven't made any visit requests matching this criteria yet.</p>
                    <Link :href="route('visitor.dashboard')" class="inline-block mt-4 text-indigo-600 font-bold hover:underline">
                        Request a New Visit →
                    </Link>
                </div>

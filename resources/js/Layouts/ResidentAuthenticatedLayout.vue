    ...
    <div class="flex min-h-screen bg-gray-100 dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors duration-200">
        ...
        <aside class="w-64 shrink-0 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 shadow-sm min-h-screen hidden md:block transition-colors duration-200">
            <div class="p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
                <Link :href="route('resident.dashboard')" class="flex items-center">
                    ...
                    <span class="ml-2 font-bold text-base text-gray-800 dark:text-white">My Home</span>
                </Link>
                <ThemeToggle />
            </div>
            ...
            <nav class="mt-6 px-4 space-y-2">
                ...
                <Link :href="route('resident.inquiries.index')" class="flex items-center px-4 py-3 text-gray-600 dark:text-gray-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 hover:text-indigo-700 dark:hover:text-indigo-400 rounded-lg transition-all duration-200" :class="{ 'bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-400 font-bold shadow-sm': route().current('resident.inquiries.*') }">
                    <span class="mr-3 text-xl">💬</span> Contact Us
                </Link>

                <div class="border-t border-gray-200 dark:border-gray-800 my-4"></div>

                <Link :href="route('resident.manual.index')" class="flex items-center px-4 py-3 text-gray-600 dark:text-gray-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 hover:text-indigo-700 dark:hover:text-indigo-400 rounded-lg transition-all duration-200" :class="{ 'bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-400 font-bold shadow-sm': route().current('resident.manual.index') }">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-sm md:hidden flex justify-between items-center p-4">
                <Link :href="route('resident.dashboard')" class="flex items-center gap-2">
                    ...
                    <span class="font-bold text-gray-800 dark:text-white">My Home</span>
                </Link>
             ...
             <!-- Mobile Menu Dropdown -->
              <div v-if="showingNavigationDropdown" class="md:hidden bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 p-4 space-y-2">
                  ...
                  <Link :href="route('resident.inquiries.index')" class="block py-2 text-gray-700 dark:text-gray-300 font-semibold" :class="{ 'text-indigo-600 dark:text-indigo-400': route().current('resident.inquiries.*') }">Contact Us</Link>
                  <Link :href="route('resident.manual.index')" class="block py-2 text-gray-700 dark:text-gray-300 font-semibold" :class="{ 'text-indigo-600 dark:text-indigo-400': route().current('resident.manual.index') }">User Manual</Link>
